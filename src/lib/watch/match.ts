...
export function generateWatchRules(
  ...
  const watchList = Array.isArray(watchPaths)
    ? watchPaths
    : watchPaths
      ? [watchPaths]
      : []
  const ignoreList = Array.isArray(ignorePaths)
    ? ignorePaths
    : ignorePaths
      ? [ignorePaths]
      : []
...
export function filterFilesByMonitorRules(
  files: string[],
  monitor: string[],
  ext: string
): MonitorResult {
  ...
  if (ext) {
    ...
    good = good.filter(file =>
      minimatch(basename(file), ext as string, minimatchOpts)
    )
  }
