...
// ***All the code is sourced from "Acorn", with error warnings "removed" to better highlight the code itself.***
class Position {
  constructor(
    public line: number,
...
export default class Tokenizer {
  ...
  readEscapedChar(inTemplate: boolean) {
    let ch = this.input.charCodeAt(++this.pos)
    ++this.pos
    switch (ch) {
      case 110:
  ...
  readToken_mult_modulo_exp(code: number) {
    // '%*'
    let next = this.input.charCodeAt(this.pos + 1)
    let size = 1
  ...
  readToken_plus_min(code: number) {
    // '+-'
    let next = this.input.charCodeAt(this.pos + 1)
    if (next === code) {
      if (
