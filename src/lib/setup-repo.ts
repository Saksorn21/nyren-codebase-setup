...
export async function setPrettierJson(
  target: string,
  src: string = process.cwd()
): Promise<ResultFs> {
  const repoPath = resolvePath(
    __dirname,
    '../../repo-templates/.prettierrc.json'
  )
  ...
  const { success, error } = await createJsonFile(
    resolvePath(src, '.prettierrc.json'),
    parsedData
  )
  if (error) {
...
export async function createFileMain(
  target: string,
  path: string,
  basePath: string = process.cwd()
) {
  const basePathUser = resolvePath(
    basePath,
    `${path}/index${path !== 'src' ? '.test.' : '.'}${target === 'typescript' ? 'ts' : 'js'}`
  )
  const dataMain = `
