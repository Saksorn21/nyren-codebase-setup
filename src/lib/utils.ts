import { help, tools } from './help.js'
export async function fetchToJson(url: string): Promise<Record<string, string>> {
  try {
    ...
    tools.log(
      tools.error,
       tools.textRed(`Failed to unable to connect: ${tools.textWhit((e as Error).message)}`))
    return {}
  }
}
