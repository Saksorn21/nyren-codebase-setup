...
const findMatchingScript = async (commandArgs: string[], directory?: string): Promise<{ result: MatchResult , commandArgs: string[] }> => {
  try {
    const directoryPackageJson = directory ? resolvePath(directory, 'package.json') : 'package.json'
    const scripts = readPackageJson(resolvePath(process.cwd(), directoryPackageJson)).scripts;
...
const prepareScriptCommand = async (commandArgs: string[]): Promise<void> => { 
  ...
  const isValidCommand = command.endsWith('ts') || command.endsWith('js') || command.endsWith('cjs') || command.endsWith('mjs');

  if (!isValidCommand) {
    t.log(t.prefixCli, t.error, t.text('#EF3054')(`The command: ${command} is not a valid script. Please provide a valid script file: ${t.textWhit(`path/to/file.<ts | js | cjs | mjs>`)}`));
    process.exit(1);
  }
...
export async function executeScriptDynamic(
  ...
  const { result: scriptMatchResult, commandArgs } = await findMatchingScript([script],options.prefix);
  ...
  t.log(t.prefixCli,t.text('#8390FA')(`${t.toolIcon} run ${t.textWhit(script)} ${t.text('#6B1D70')('$')} ${t.textWhit.dim(commandArgsResult.join(' '))}`))
